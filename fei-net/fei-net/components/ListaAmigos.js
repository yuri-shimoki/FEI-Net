...
export default function ListaAmigos({ route, navigation })
{
  ...
  useEffect(() => {
    ...
    ref.on('value', snapshot => {
      ...
      if (dados !== null) {
        const arrayDeAmigos = Object.keys(dados).map(chave => ({
              nome: chave,
              ...dados[chave]
              }))
              .filter(amigo => amigo.nome !== nomeDeUsuario);
...
function RenderizarAmigo({ item, nomeDeUsuario, navigation })
{
  ...
  {
    const ref = firebase.database().ref(`/usuarios/${nomeDeUsuario}/amigos/${nomeAmigo}`);
